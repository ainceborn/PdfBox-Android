...
import com.tom_roush.fontbox.ttf.model.Language;
import com.tom_roush.fontbox.ttf.model.MapBackedGsubData;
...
import com.tom_roush.fontbox.ttf.table.common.LookupListTable;
...
public class GlyphSubstitutionDataExtractor
{
     ...
     * {@link com.tom_roush.fontbox.ttf.model.Language}'s searching for the first match with the scripts of the font. Instead, it unconditionally
     * creates {@link ScriptTableDetails} instance with language left {@linkplain com.tom_roush.fontbox.ttf.model.Language#UNSPECIFIED unspecified}.
     * 
     * @param scriptName
     * @param scriptTable
    ...
    private ScriptTableDetails getSupportedLanguage(Map<String, ScriptTable> scriptList)
    {
        for (Language lang : Language.values())
        {
            for (String scriptName : lang.getScriptNames())
            {
                ...
                if (value != null)
                {
                    Log.d("PdfBox-Android", String.format(
                            "Language decided: %s %s",
                            lang,
                            scriptName
                    ));
            ...
            LookupTable lookupTable)
    {
        ...
        for (LookupSubTable lookupSubTable : lookupTable.getSubTables())
        {
            ...
            else
            {
                ...
                Log.e("PdfBox-Android", "The type {"+lookupSubTable+"} is not yet supported, will be ignored");
            }
        }
            ...
            LookupTypeSingleSubstFormat2 singleSubstTableFormat2)
    {
        ...
        {
            Log.w("PdfBox-Android", String.format(
                    "The coverage table size (%d) should be the same as the count of the substituteGlyphIDs tables (%d)",
            ...
            LookupTypeAlternateSubstitutionFormat1 alternateSubstitutionFormat1)
    {
        ...
        {
            Log.w("PdfBox-Android", String.format(
                    "The coverage table size (%d) should be the same as the count of the alternate set tables (%d)",
                    coverageTable.getSize(),
            ...
            List<Integer> newGlyphList, List<Integer> glyphsToBeSubstituted)
    {
        ...
        {
            Log.d("PdfBox-Android", String.format(
                    "For the newGlyph: %s, newValue: %s is trying to override the oldValue %s",
