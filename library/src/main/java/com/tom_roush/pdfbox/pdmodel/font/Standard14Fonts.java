...
public final class Standard14Fonts
{
    ...
    private static void loadMetrics(FontName fontName) throws IOException
    {
        ...
        if (PDFBoxResourceLoader.isReady())
        {
            ...
            resourceAsStream = new BufferedInputStream(Standard14Fonts.class.getResourceAsStream("/" + resourceName));
        }
    ...
    private static FontBoxFont getMappedFont(FontName baseName)
    {
        if (!GENERIC_FONTS.containsKey(baseName))
        {
            synchronized (GENERIC_FONTS)
            {
                if (!GENERIC_FONTS.containsKey(baseName))
                {
                    PDType1Font type1Font = null;
                    type1Font = new PDType1Font(baseName);
     ...
     */
    public static Path getGlyphPath(FontName baseName, String glyphName) throws IOException
    {
