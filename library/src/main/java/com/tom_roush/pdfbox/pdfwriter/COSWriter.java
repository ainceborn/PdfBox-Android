...
import com.tom_roush.pdfbox.pdfparser.xref.FreeXReference;
import com.tom_roush.pdfbox.pdfparser.xref.NormalXReference;
import com.tom_roush.pdfbox.pdfparser.xref.ObjectStreamXReference;
import com.tom_roush.pdfbox.pdfparser.xref.XReferenceEntry;
import com.tom_roush.pdfbox.pdfwriter.compress.COSWriterCompressionPool;
import com.tom_roush.pdfbox.pdfwriter.compress.COSWriterObjectStream;
                    ...
                    if (base2 instanceof COSInteger)
                    {
                        ...
                        if (br2 > incrementalInput.length())
                        {
                            Log.d("PdfBox-Android",
                                    "reachedSignature at offset {" + getStandardOutput().getPos() + "} " +
                                    " byteRange: {" +byteRange + "} " +
                                    "input length: {" + incrementalInput.length() +"}");
